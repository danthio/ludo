...
def draw_bg():
	...
	im=Image.open("data/dice_wallpaper.jpg")
	x,y=im.size

	xv=(590-x)/2

	bg2=ImageTk.PhotoImage(file="data/dice_wallpaper.jpg")

	can.create_image(xv,0,image=bg2,anchor="nw")
	...
	if game_mode=="classic" and conv==0:
		...
		if turn==game1[0]:
			...
			can.create_polygon(70,590-100-70, 70,590-70, 70+100,590-70, 70+10,590-70-10,fill=cl,outline=cl)
			...
			can.create_polygon(70,70+100, 70,70, 70+100,70, 70+10,70+10,  fill=cl,outline=cl)
			...
			can.create_polygon(590-70-100,70, 590-70,70, 590-70,70+100, 590-70-10,70+10, 590-70-100,70,  fill=cl,outline=cl)
			...
			can.create_polygon(590-70,580-70-100, 590-70,590-70, 590-70-100,590-70, 590-70-10,590-70-10, 590-70,580-70-100,  fill=cl,outline=cl)
	...
	for i in game2:
		...
		if len(game2[i])==2:
			...
			if int(p)==1:
				...
				draw_pointer(x_+_s+s/2,y_+_s+s/2,40,col)

			elif int(p)==2:
				draw_pointer(x_+_s*2+s+s/2,y_+_s+s/2,40,col)

			elif int(p)==3:
				draw_pointer(x_+_s+s/2,y_+_s*2+s+s/2,40,col)
			elif int(p)==4:
				draw_pointer(x_+_s*2+s+s/2,y_+_s*2+s+s/2,40,col)
...
def sel_game_mode():
	...
	can.create_arc(wd/2-100,ht/2-30, wd/2-100+30,ht/2,style="arc",start=90,extent=180,outline="#f5f5f5",width=1)
	can.create_arc(wd/2+100-30,ht/2-30,wd/2+100,ht/2,style="arc",start=270,extent=180,outline="#f5f5f5",width=1)


	can.create_line(wd/2-100+15,ht/2-30,  wd/2+100-15,ht/2-30,fill="#f5f5f5",width=1)
	can.create_line(wd/2-100+15,ht/2,  wd/2+100-15,ht/2,fill="#f5f5f5",width=1)



	can.create_arc(wd/2-100,ht/2-30+60, wd/2-100+30,ht/2+60,style="arc",start=90,extent=180,outline="#f5f5f5",width=1)
	can.create_arc(wd/2+100-30,ht/2-30+60,wd/2+100,ht/2+60,style="arc",start=270,extent=180,outline="#f5f5f5",width=1)


	can.create_line(wd/2-100+15,ht/2-30+60,  wd/2+100-15,ht/2-30+60,fill="#f5f5f5",width=1)
	can.create_line(wd/2-100+15,ht/2+60,  wd/2+100-15,ht/2+60,fill="#f5f5f5",width=1)




	can.create_text(wd/2,ht/2-30+15,text="Classic",font=("FreeMono",13),fill="#f5f5f5")
	can.create_text(wd/2,ht/2-30+15+60,text="Rush Mode",font=("FreeMono",13),fill="#f5f5f5")
...
def sel_col():
	...
	for _ in range(4):
		...
		can.create_oval(x_,ht/2-50, x_+100,ht/2+50,outline="#444444")
	...
	can.create_arc(wd/2-100,ht-50-30, wd/2-100+30,ht-50,start=90,extent=180,style="arc",outline="#f5f5f5")
	can.create_arc(wd/2+100-30,ht-50-30, wd/2+100,ht-50,start=270,extent=180,style="arc",outline="#f5f5f5")


	can.create_line(wd/2-100+15,ht-50-30, wd/2+100-15,ht-50-30,fill="#f5f5f5") 
	can.create_line(wd/2-100+15,ht-50, wd/2+100-15,ht-50,fill="#f5f5f5")

	can.create_text(wd/2,ht-50-30+15,text="Start Game",font=("FreeMono",13),fill="#f5f5f5")		
...
def draw_pointer(x,y,sz,col):
	...
	can.create_polygon(ar,fill=col2,outline=col2)
	can.create_line(ar2,fill=col3)

	can.create_oval(cx-sz/5*1.1,cy-sz/5*1.1, cx+sz/5*1.1,cy+sz/5*1.1, fill=col3,outline=col3)
